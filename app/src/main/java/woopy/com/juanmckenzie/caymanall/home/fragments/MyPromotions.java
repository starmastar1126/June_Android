...
import woopy.com.juanmckenzie.caymanall.home.adapters.MyPromotionAdapter;
...
public class MyPromotions extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.add).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                startActivity(new Intent(getApplicationContext(), PurchasePromotion.class));
            }
        });
        ...
        mlTitleTxt.setText(getString(R.string.my_promotions));
    ...
    List<promotions> MyAds = new ArrayList<>();

    // MARK: - QUERY MY ADS ------------------------------------------------------------------
    private void queryMyAds() {
        dialog.show();

        FirebaseDatabase.getInstance().getReference().child("promotionsall").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {

                MyAds = new ArrayList<>();
                for (DataSnapshot dataSnapshot1 : dataSnapshot.getChildren()) {
                    promotions ads = dataSnapshot1.getValue(promotions.class);
                    if (ads.getSellerPointer() != null) {
                        if (ads.getSellerPointer().getFirebaseID().equals(myapplication.getCurrentUser().getFirebaseID())) {
                            ...
                            if (AdsListActivity.getUnsignedDiffInDays(date, Calendar.getInstance().getTime()) < 30) {
                                MyAds.add(ads);
                            }
                        }
                    }
                ...
                if (MyAds.isEmpty()) {
                    ...
                    myAdsRV.setLayoutManager(new LinearLayoutManager(MyPromotions.this));
                    myAdsRV.setAdapter(new MyPromotionAdapter(MyAds, MyPromotions.this, new MyPromotionAdapter.OnMyAdClickListener() {
                        @Override
                        public void onAdClicked(promotions adObject) {
    ...
    private void startSellEditItemScreen(promotions adObjid) {
        if (adObjid != null) {
            myapplication.setSelectePromotions(adObjid);
            Intent i = new Intent(MyPromotions.this, PurchasePromotion.class);
